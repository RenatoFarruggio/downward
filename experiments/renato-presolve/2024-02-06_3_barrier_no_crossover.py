...
CONFIGS = [
    ...
    (f"config_SEH_barrier", ["--search", "astar(operatorcounting([state_equation_constraints()],lp_solve_method=4,crossover=false))"]),
    
    (f"config_DEL_default", ["--search", "astar(operatorcounting([delete_relaxation_constraints()]))"]),
    (f"config_DEL_barrier", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lp_solve_method=4,crossover=false))"]),
    
    (f"config_OCP_default", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal))"]),
    (f"config_OCP_barrier", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lp_solve_method=4,crossover=false))"]),
    
    (f"config_PHO_default", ["--search", "astar(operatorcounting([pho_constraints()]))"]),
    (f"config_PHO_barrier", ["--search", "astar(operatorcounting([pho_constraints()],lp_solve_method=4,crossover=false))"]),
    
    (f"config_IPOT_default", ["--search", "astar(initial_state_potential())"]),
    (f"config_IPOT_barrier", ["--search", "astar(initial_state_potential(lp_solve_method=4,crossover=false))"]),
    
    (f"config_DPOT_default", ["--search", "astar(diverse_potentials())"]),
    (f"config_DPOT_barrier", ["--search", "astar(diverse_potentials(lp_solve_method=4,crossover=false))"]),
]

BUILD_OPTIONS = []
REV_NICKS = [
    ("symmetrybreaking", "barrier_no_crossover_vs_default"),
]
ATTRIBUTES = ["error",
    "run_dir",
    #"search_start_time",
