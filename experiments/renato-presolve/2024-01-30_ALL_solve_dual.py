...
CONFIGS = [
    (f"config_SEH_solve_dual_automatic", ["--search", "astar(operatorcounting([state_equation_constraints()],solve_dual=0))"]),
    (f"config_SEH_solve_dual_on", ["--search", "astar(operatorcounting([state_equation_constraints()],solve_dual=1))"]),
    
    (f"config_DEL_solve_dual_automatic", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],solve_dual=0))"]),
    (f"config_DEL_solve_dual_on", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],solve_dual=1))"]),
    
    (f"config_OCP_solve_dual_automatic", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,solve_dual=0))"]),
    (f"config_OCP_solve_dual_on", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,solve_dual=1))"]),
    
    (f"config_PHO_solve_dual_automatic", ["--search", "astar(operatorcounting([pho_constraints()],solve_dual=0))"]),
    (f"config_PHO_solve_dual_on", ["--search", "astar(operatorcounting([pho_constraints()],solve_dual=1))"]),
    
    (f"config_IPOT_solve_dual_automatic", ["--search", "astar(initial_state_potential(use_presolve=false,solve_dual=0))"]),
    (f"config_IPOT_solve_dual_on", ["--search", "astar(initial_state_potential(use_presolve=false,solve_dual=1))"]),
    
    (f"config_DPOT_solve_dual_automatic", ["--search", "astar(diverse_potentials(use_presolve=false,solve_dual=0))"]),
    (f"config_DPOT_solve_dual_on", ["--search", "astar(diverse_potentials(use_presolve=false,solve_dual=1))"]),
]

BUILD_OPTIONS = []
DRIVER_OPTIONS = ["--overall-time-limit", "5m"]
REV_NICKS = [
    ("symmetrybreaking", "solve_dual"),
]
ATTRIBUTES = ["error",
    "run_dir",
    #"search_start_time",
