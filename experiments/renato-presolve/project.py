...
WARM_STARTS_PER_EVALUATIONS = Attribute(
    "warm_starts_per_evaluations", min_wins=False, function=arithmetic_mean, digits=1
)
...
PRESOLVE_TIME_PER_LP_SOLVE_TIME_SUM = Attribute(
    "presolve_time_per_lp_solve_time_sum",
    min_wins=True, function=arithmetic_mean, digits=5
)
...
def add_warm_starts_per_evaluations(run):
    ...
    evaluations = run.get("evaluations")
    if num_warm_starts is not None and evaluations is not None and evaluations > 0:
        run["warm_starts_per_evaluations"] = num_warm_starts / evaluations
    return run
...
def add_sparsity(run):
    ...
    if lp_variables is not None and lp_constraints is not None and lp_nonzero_entries is not None and size > 0:
        run["sparsity"] = 1- (lp_nonzero_entries / size)
    return run


def add_lp_count(run):
    total_zero_iterations_count = run.get("total_zero_iterations_count")
    total_nonzero_iterations_count = run.get("total_nonzero_iterations_count")
    if total_zero_iterations_count is not None and total_nonzero_iterations_count is not None:
        lp_count = total_zero_iterations_count + total_nonzero_iterations_count
        if lp_count is not None:
            run["lp_count"] = lp_count
        else:
            run["lp_count"] = 0
    else:
        run["lp_count"] = 0
...
def add_average_iterations_after_initial_per_initial_iterations(run):
    average_iterations_after_initial = run.get("average_iterations_after_initial")
...
def add_initial_iterations_per_average_iterations_after_initial(run):
    initial_iterations_total = run.get("initial_iterations_total")
    average_iterations_after_initial = run.get("average_iterations_after_initial")
    if average_iterations_after_initial is not None and initial_iterations_total is not None and average_iterations_after_initial > 0:
        run["initial_iterations_per_average_iterations_after_initial"] = initial_iterations_total / average_iterations_after_initial
    else:
        run["initial_iterations_per_average_iterations_after_initial"] = None
    return run

def add_presolve_time_per_lp_solve_time_sum(run):
    presolve_time = run.get("presolve_time")
    lp_solve_time_sum = run.get("lp_solve_time_sum")
    if presolve_time is not None and lp_solve_time_sum is not None and lp_solve_time_sum > 0:
        run["presolve_time_per_lp_solve_time_sum"] = presolve_time / lp_solve_time_sum
    else:
        run["presolve_time_pre_lp_solve_time_sum"] = None
    return run
