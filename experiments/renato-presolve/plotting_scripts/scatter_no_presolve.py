...
for key in data:
    ...
    if algorithm == f"config_{algo_name}_default":
        algorithm = "default"
    if algorithm == f"config_{algo_name}_no_preprocessing":
        algorithm = "barrier"
...
plt.title(f'Time needed per task with {algo_name} heuristic no preprocessing vs default')
...
plt.savefig(f'scatter_{algo_name}_no_preprocessing_vs_default.png', dpi=300)
