...
CONFIGS = [
    (f"config_state_equation_heuristic", ["--search", "astar(operatorcounting([state_equation_constraints()],lp_solve_method=2))"]),
    (f"config_delete_relaxation_heuristic", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lp_solve_method=2))"]),
    (f"config_optimal_cost_partitioning_heuristic", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lp_solve_method=2))"]),
    (f"config_pho_heuristic", ["--search", "astar(operatorcounting([pho_constraints()],lp_solve_method=2))"]),
    (f"config_initial_potential_heuristic", ["--search", "astar(initial_state_potential(use_presolve=false,lp_solve_method=2))"]),
    (f"config_diverse_potential_heuristic", ["--search", "astar(diverse_potentials(use_presolve=false,lp_solve_method=2))"]),
]
...
project.add_comparative_report(
    exp,
    algorithm_pairs = [(algorithms[0])],
    attributes=["iterations_total"],
    name_postfix="TEST_comparative_report"
)
