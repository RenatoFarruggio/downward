...
CONFIGS = [
    ...
    (f"config_SEH_no_presolve", ["--search", "astar(operatorcounting([state_equation_constraints()],use_presolve=false))"]),
    
    (f"config_DEL_default", ["--search", "astar(operatorcounting([delete_relaxation_constraints()]))"]),
    (f"config_DEL_no_presolve", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],use_presolve=false))"]),
    
    (f"config_OCP_default", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal))"]),
    (f"config_OCP_no_presolve", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,use_presolve=false))"]),
    
    (f"config_PHO_default", ["--search", "astar(operatorcounting([pho_constraints()]))"]),
    (f"config_PHO_no_presolve", ["--search", "astar(operatorcounting([pho_constraints()],use_presolve=false))"]),
    
    (f"config_IPOT_default", ["--search", "astar(initial_state_potential())"]),
    (f"config_IPOT_no_presolve", ["--search", "astar(initial_state_potential(use_presolve=false))"]),
    
    (f"config_DPOT_default", ["--search", "astar(diverse_potentials())"]),
    (f"config_DPOT_no_presolve", ["--search", "astar(diverse_potentials(use_presolve=false))"]),
]

BUILD_OPTIONS = []
REV_NICKS = [
    ("symmetrybreaking", "no_presolve_vs_default"),
]
ATTRIBUTES = ["error",
    "run_dir",
    #"search_start_time",
