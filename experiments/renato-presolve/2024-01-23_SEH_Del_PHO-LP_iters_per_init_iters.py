...
CONFIGS = [
    ...
    (f"config_state_equation_heuristic", ["--search", "astar(operatorcounting([state_equation_constraints()],lpsolver=cplex))"]),  # state equation heuristic
    #(f"config_cplex_twophase_state_equation_heuristic", ["--search", "astar(operatorcounting([state_equation_constraints()],lpsolver=cplex_twophase))"]),  # state equation heuristic
    
    # Delete Relaxation Heuristic
    (f"config_delete_relaxation_heuristic", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lpsolver=cplex))"]),  # delete relaxation heuristic
    #(f"config_cplex_twophase_delete_relaxation_heuristic", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lpsolver=cplex_twophase))"]),  # delete relaxation heuristic
    
#    # Landmark Optimal Cost Partitioning Heuristic
#    (f"config_cplex_standard_landmark_optimal_cost_partitioning_heuristic", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lpsolver=cplex))"]),
#    #(f"config_cplex_twophase_landmark_optimal_cost_partitioning_heuristic", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lpsolver=cplex_twophase))"]),

    # Post Hoc Optimization Heuristic
    (f"config_pho_heuristic", ["--search", "astar(operatorcounting([pho_constraints()],lpsolver=cplex))"]),
    #(f"config_cplex_twophase_pho_heuristic", ["--search", "astar(operatorcounting([pho_constraints()],lpsolver=cplex_twophase))"]),

#    # Initial Potential Heuristic (no presolve)
#    (f"config_cplex_standard_init_potential_heuristic", ["--search", "astar(initial_state_potential(use_presolve=false,lpsolver=cplex))"]),
#    (f"config_cplex_twophase_init_potential_heuristic", ["--search", "astar(initial_state_potential(use_presolve=false,lpsolver=cplex_twophase))"]),
#
#    # Diverse Potential Heuristic (no presolve)
#    (f"config_cplex_standard_diverse_potential_heuristic", ["--search", "astar(diverse_potentials(use_presolve=false,lpsolver=cplex))"]),
#    (f"config_cplex_twophase_diverse_potential_heuristic", ["--search", "astar(diverse_potentials(use_presolve=false,lpsolver=cplex_twophase))"]),
