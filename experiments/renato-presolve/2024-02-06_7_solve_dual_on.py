...
CONFIGS = [
    ...
    (f"config_IPOT_solve_dual_on", ["--search", "astar(initial_state_potential(solve_dual=1))"]),
    
    (f"config_DPOT_default", ["--search", "astar(diverse_potentials())"]),
    (f"config_DPOT_solve_dual_on", ["--search", "astar(diverse_potentials(solve_dual=1))"]),
]

BUILD_OPTIONS = []
REV_NICKS = [
    ("symmetrybreaking", "solve_dual_on_vs_default"),
]
ATTRIBUTES = ["error",
    "run_dir",
    #"search_start_time",
