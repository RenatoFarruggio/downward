...
#    (f"SEH", ["--search", "astar(operatorcounting([state_equation_constraints()],use_presolve=false,aggregator_application_limit=0),bound=0)"]),
    (f"default",                    ["--search", "astar(operatorcounting([delete_relaxation_constraints()]))"]),
    (f"use_time_vars",              ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_time_vars=true)]))"]),
    (f"use_integer_vars",           ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_integer_vars=true)], use_integer_operator_counts=true))"]),
    (f"use_integer_time_vars",      ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_time_vars=true, use_integer_vars=true)], use_integer_operator_counts=true))"]),
    (f"default_barrier",                    ["--search", "astar(operatorcounting([delete_relaxation_constraints()], lp_solve_method=4))"]),
    (f"use_time_vars_barrier",              ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_time_vars=true)], lp_solve_method=4))"]),
    (f"use_integer_vars_barrier",           ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_integer_vars=true)], use_integer_operator_counts=true, lp_solve_method=4))"]),
    (f"use_integer_time_vars_barrier",      ["--search", "astar(operatorcounting([delete_relaxation_constraints(use_time_vars=true, use_integer_vars=true)], use_integer_operator_counts=true, lp_solve_method=4))"]),
#    (f"default", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal))"]),
#    (f"barrier", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lp_solve_method=4))"]),
...
REV_NICKS = [
    ...
    ("symmetrybreaking", "OCP_barrier_vs_default"),
]
