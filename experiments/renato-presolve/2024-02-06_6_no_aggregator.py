...
CONFIGS = [
    ...
    (f"config_SEH_no_aggregator", ["--search", "astar(operatorcounting([state_equation_constraints()],aggregator_application_limit=0))"]),
    
    (f"config_DEL_default", ["--search", "astar(operatorcounting([delete_relaxation_constraints()]))"]),
    (f"config_DEL_no_aggregator", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],aggregator_application_limit=0))"]),
    
    (f"config_OCP_default", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal))"]),
    (f"config_OCP_no_aggregator", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,aggregator_application_limit=0))"]),
    
    (f"config_PHO_default", ["--search", "astar(operatorcounting([pho_constraints()]))"]),
    (f"config_PHO_no_aggregator", ["--search", "astar(operatorcounting([pho_constraints()],aggregator_application_limit=0))"]),
    
    (f"config_IPOT_default", ["--search", "astar(initial_state_potential())"]),
    (f"config_IPOT_no_aggregator", ["--search", "astar(initial_state_potential(aggregator_application_limit=0))"]),
    
    (f"config_DPOT_default", ["--search", "astar(diverse_potentials())"]),
    (f"config_DPOT_no_aggregator", ["--search", "astar(diverse_potentials(aggregator_application_limit=0))"]),
]

BUILD_OPTIONS = []
REV_NICKS = [
    ("symmetrybreaking", "no_aggregator_vs_default"),
]
ATTRIBUTES = ["error",
    "run_dir",
    #"search_start_time",
