...
REV_NICKS = [
    ...
    ("symmetrybreaking", "reductions"),
]

PRESOLVE_TIME_PER_TOTAL_TIME = Attribute(
    "presolve_time_per_total_time", min_wins=True, function=arithmetic_mean, digits=2
)
VARIABLES_PER_CONSTRAINT_OF_PRESOLVED = Attribute(
    "variables_per_constraint_of_presolved", min_wins=True, function=arithmetic_mean, digits=2
)
CONSTRAINTS_PER_VARIABLE_OF_PRESOLVED = Attribute(
    "constraints_per_variable_of_presolved", min_wins=True, function=arithmetic_mean, digits=2
)
...
ATTRIBUTES = [
    ...
    PRESOLVE_TIME_PER_TOTAL_TIME,
    VARIABLES_PER_CONSTRAINT_OF_PRESOLVED,
...
def add_presolve_time_per_total_time(run):
    ...
    if total_time is not None and presolve_time is not None and total_time > 0:
        run["presolve_time_per_total_time"] = presolve_time / total_time
    return run
...
def add_variables_per_constraint_of_presolved(run):
    ...
    if lp_variables_of_reduced is not None and lp_constraints_of_reduced is not None:
        if lp_constraints_of_reduced > 0:
            run["variables_per_constraint_of_presolved"] = lp_variables_of_reduced / lp_constraints_of_reduced
        else:
            run["variables_per_constraint_of_presolved"] = None
    return run

def add_constraints_per_variable_of_presolved(run):
    lp_variables_of_reduced = run.get("lp_variables_of_reduced")
    lp_constraints_of_reduced = run.get("lp_constraints_of_reduced")
    if lp_variables_of_reduced is not None and lp_constraints_of_reduced is not None:
        if lp_variables_of_reduced > 0:
            run["constraints_per_variable_of_presolved"] = lp_constraints_of_reduced / lp_variables_of_reduced
        else:
            run["constraints_per_variable_of_presolved"] = None
    return run
...
project.add_absolute_report(
    ...
    filter=[add_presolve_time_per_total_time,
            add_variables_per_constraint_of_presolved,
            add_constraints_per_variable_of_presolved],
...
for algorithm in algorithms:
    project.add_absolute_report(
        ...
        filter=[remove_failed_runs,
                add_presolve_time_per_total_time,
                add_variables_per_constraint_of_presolved,
                add_constraints_per_variable_of_presolved],
