...
CONFIGS = [
    ...
    #(f"config_DEL_crossover_disabled", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lp_solve_method=4,crossover=false))"]),
    ...
    #(f"config_OCP_crossover_disabled", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lp_solve_method=4,crossover=false))"]),
    
    #(f"config_PHO_crossover_enabled", ["--search", "astar(operatorcounting([pho_constraints()],lp_solve_method=4,crossover=true))"]),
    #(f"config_PHO_crossover_disabled", ["--search", "astar(operatorcounting([pho_constraints()],lp_solve_method=4,crossover=false))"]),
    
    #(f"config_IPOT_crossover_enabled", ["--search", "astar(initial_state_potential(use_presolve=false,lp_solve_method=4,crossover=true))"]),
    #(f"config_IPOT_crossover_disabled", ["--search", "astar(initial_state_potential(use_presolve=false,lp_solve_method=4,crossover=false))"]),
    
    #(f"config_DPOT_crossover_enabled", ["--search", "astar(diverse_potentials(use_presolve=false,lp_solve_method=4,crossover=true))"]),
    #(f"config_DPOT_crossover_disabled", ["--search", "astar(diverse_potentials(use_presolve=false,lp_solve_method=4,crossover=false))"]),
]
