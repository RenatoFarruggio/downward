...
CONFIGS = [
    ...
    (f"config_SEH_cplex_twophase", ["--search", "astar(operatorcounting([state_equation_constraints()],lpsolver=cplex_twophase))"]),
    
    (f"config_DEL_default", ["--search", "astar(operatorcounting([delete_relaxation_constraints()]))"]),
    (f"config_DEL_cplex_twophase", ["--search", "astar(operatorcounting([delete_relaxation_constraints()],lpsolver=cplex_twophase))"]),
    
    (f"config_OCP_default", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal))"]),
    (f"config_OCP_cplex_twophase", ["--search", "astar(landmark_cost_partitioning(lm_rhw(),cost_partitioning=optimal,lpsolver=cplex_twophase))"]),
    
    (f"config_PHO_default", ["--search", "astar(operatorcounting([pho_constraints()]))"]),
    (f"config_PHO_cplex_twophase", ["--search", "astar(operatorcounting([pho_constraints()],lpsolver=cplex_twophase))"]),
    
    (f"config_IPOT_default", ["--search", "astar(initial_state_potential())"]),
    (f"config_IPOT_cplex_twophase", ["--search", "astar(initial_state_potential(lpsolver=cplex_twophase))"]),
    
    (f"config_DPOT_default", ["--search", "astar(diverse_potentials())"]),
    (f"config_DPOT_cplex_twophase", ["--search", "astar(diverse_potentials(lpsolver=cplex_twophase))"]),
]
