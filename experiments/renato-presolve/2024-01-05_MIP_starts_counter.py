...
REV_NICKS = [
    ("symmetrybreaking", "mip_starts_counter"),
]
ATTRIBUTES = [
    "error",
    "run_dir",
...
def add_warm_starts_per_evaluations(run):
    num_warm_starts = run.get("num_warm_starts")
    evaluations = run.get("evaluations")
    if num_warm_starts is not None and evaluations is not None and evaluations > 1:
        run["warm_starts_per_evaluations"] = num_warm_starts / (evaluations - 1) 
    return run
...
project.add_absolute_report(
    exp,
    attributes=ATTRIBUTES + ["warm_starts_plus_cold_starts_minus_evaluations", "warm_starts_per_evaluations", "repairs_per_warm_starts"], 
    ...
    filter=[add_warm_starts_plus_cold_starts_minus_evaluations, 
            add_warm_starts_per_evaluations,
